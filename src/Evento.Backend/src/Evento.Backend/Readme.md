...
WORKDIR /src
COPY ["Evento.Backend.csproj", "Evento.Backend/"]
RUN dotnet restore "Evento.Backend/Evento.Backend.csproj"

WORKDIR "/src/Evento.Backend"
COPY . .
RUN dotnet build "Evento.Backend.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Evento.Backend.csproj" \
...
Execute unit tests
```
cd "Evento.Backend/test/Evento.Backend.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "Evento.Backend/src/Evento.Backend"
